...
- **字数要求**：单个任务卡内容字数不得少于原PRD文档的75%
- **背景信息**：必须完整，包含业务上下文、技术背景、相关模块说明
- **目标描述**：必须明确具体的功能点、边界条件、异常处理要求
- **验收要求**：必须可量化、可验证，包含具体的测试场景
- **技术方案**：必须包含实现思路、关键步骤、涉及的文件和模块
...
4. **必须在任务卡末尾添加「任务完成报告」章节**

### 任务完成报告要求（重要）

任务完成后，必须在任务卡中添加以下格式的完成报告：

---BEGIN COMPLETION_REPORT_TEMPLATE---
## 任务完成报告
...
| 文件路径 | 改动类型 | 改动说明 |
|----------|----------|----------|
| `src/xxx/file.ts` | 新增/修改/删除 | 具体改动内容描述 |
...
> 其他需要说明的信息，如已知问题、后续优化建议等
---END COMPLETION_REPORT_TEMPLATE---
